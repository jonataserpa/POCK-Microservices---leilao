...
export function Header({
  ...
  const styles = {
    backgroundColor:
      theme?.styles?.backgroundColor || "rgba(255, 255, 255, 0.95)",
  ...
  const Menu = () => (
    <nav
      className={`hidden md:flex space-x-6 ${theme?.menu?.position === "center" ? "mx-auto" : ""}`}
    >
      {(
        theme?.menu?.items || [
  ...
  const Actions = () => (
    <div className="flex items-center gap-3">
      ...
      <button
        className="bg-white/10 hover:bg-white/20 backdrop-blur-sm px-4 py-1.5 rounded-full text-xs font-semibold transition-all border border-white/10 hover:shadow-sm"
        style={{ color: styles.color }}
      >
        Entrar
      </button>
  ...
  return (
    <>
      ...
      >
        <div
          className={`max-w-7xl mx-auto flex ${layout === "vertical-stacked" ? "flex-col justify-center py-3 space-y-3" : "items-center justify-between h-14"} ${layout === "horizontal-center" ? "justify-center relative" : ""}`}
        >

...
import { CarCard, Car } from "@repo/ui-kit";
...
async function getCampaign(
  ...
  try {
    ...
    const res = await fetch(
      `${apiUrl}/campaigns?tenantId=${tenantId}&slug=${slug}`,
      { next: { revalidate: 60 } },
    );
...
async function getTranslations(
  ...
  try {
    const res = await fetch(
      `${apiUrl}/translations?tenantId=${tenantId}&lang=${lang}`,
      { next: { revalidate: 60 } },
    );
    if (!res.ok) return {};
        ...
        <div className="absolute inset-0 flex items-center justify-center">
          <div className="text-center px-4 max-w-4xl">
            ...
            <div className="inline-flex items-center bg-white/10 backdrop-blur-md border border-white/20 rounded-full px-6 py-2 text-white">
              ...
              <span>
                {t("page.detail.validUntil")}:{" "}
                {new Date(campaign.validUntil).toLocaleDateString(lang)}
              </span>
            </div>
          </div>
        </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="flex items-center justify-between mb-10">
          ...
          <div className="flex gap-2">
            ...
            <a
              href={`?lang=pt-BR`}
              className={`px-3 py-1 rounded text-sm ${lang === "pt-BR" ? "bg-purple-600 text-white" : "bg-gray-200 text-gray-700"}`}
            >
              PT
            </a>
            <a
              href={`?lang=en-US`}
              className={`px-3 py-1 rounded text-sm ${lang === "en-US" ? "bg-purple-600 text-white" : "bg-gray-200 text-gray-700"}`}
            >
              EN
            </a>
            <a
              href={`?lang=es-ES`}
              className={`px-3 py-1 rounded text-sm ${lang === "es-ES" ? "bg-purple-600 text-white" : "bg-gray-200 text-gray-700"}`}
            >
              ES
            </a>
          </div>
